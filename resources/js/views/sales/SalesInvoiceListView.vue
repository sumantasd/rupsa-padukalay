<template>
  <div class="space-y-6 antialiased font-sans max-w-7xl mx-auto pb-16">
    ...
    <Teleport to="body">
      <div v-if="selectedInvoice" class="fixed inset-0 z-[500] bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
        <div class="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-4xl w-full p-6 sm:p-8 space-y-6 max-h-[90vh] overflow-y-auto text-xs">
          <div class="flex items-center justify-between border-b border-slate-200 pb-4">
            ...
            <div class="flex items-center gap-2">
              <button
                ...
                class="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold text-xs shadow-xs transition-colors flex items-center gap-1 cursor-pointer"
              >
                <span>🧾</span>
                <span>80mm Thermal Receipt</span>
              </button>
              <button @click="selectedInvoice = null" class="p-2 rounded-xl text-slate-400 hover:text-slate-800 font-black text-lg cursor-pointer">✕</button>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-200/80">
            ...
            <div class="space-y-1 text-left sm:text-right">
              <span class="font-bold text-slate-400 uppercase text-[10px] tracking-wider block">Invoice Summary</span>
              <div class="font-mono font-black text-red-600 text-sm">Grand Total: ₹{{ formatCurrency(selectedInvoice.grand_total) }}</div>
              <div class="font-bold text-emerald-700">Paid Amount: ₹{{ formatCurrency(selectedInvoice.paid_amount) }}</div>
              <div class="font-bold text-slate-600">Status: <span class="uppercase text-slate-900 font-black">{{ selectedInvoice.status }}</span></div>
            </div>
          </div>

          <div class="space-y-2">
            <h4 class="font-black text-slate-900 uppercase tracking-wider text-[11px]">Purchased Items</h4>
              ...
              <table class="w-full text-left text-xs">
                ...
                <tbody class="divide-y divide-slate-100 font-medium text-slate-800">
                  <tr v-for="item in (selectedInvoice.items || [])" :key="item.id">
                    <td class="py-2.5 px-3 font-mono font-bold text-red-600">{{ item.article_number_snapshot || item.sku_snapshot || 'N/A' }}</td>
                    <td class="py-2.5 px-3 font-bold text-slate-900">{{ item.product_name_snapshot }}</td>
                    <td class="py-2.5 px-3">{{ item.color_name_snapshot }} / IND {{ item.size_number_snapshot }}</td>
                    <td class="py-2.5 px-3 text-center font-mono font-bold">{{ item.quantity }} pcs</td>
                    <td class="py-2.5 px-3 text-right font-mono">₹{{ formatCurrency(item.unit_price) }}</td>
          ...
          <div class="flex justify-end pt-4 border-t border-slate-200">
            <button @click="selectedInvoice = null" class="px-5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold cursor-pointer">
              Close
            </button>
          </div>
        </div>
    ...
    <Teleport to="body">
      <div v-if="receiptInvoice" class="fixed inset-0 z-[500] bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
        <div class="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-sm w-full p-6 space-y-4">
          ...
          <div class="flex items-center justify-between border-b border-slate-200 pb-3">
            ...
            <button @click="receiptInvoice = null" class="text-slate-400 hover:text-slate-800 font-bold cursor-pointer">✕</button>
          </div>
          ...
          <div class="flex justify-end gap-2 pt-2 border-t border-slate-200">
            <button @click="receiptInvoice = null" class="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl font-bold text-xs cursor-pointer">
              Close
            </button>
    ...
    <Teleport to="body">
      <div v-if="deleteInvoiceTarget" class="fixed inset-0 z-[550] bg-slate-900/60 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-y-auto">
        <div class="bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl border border-slate-200 shadow-2xl p-6 space-y-5 animate-in slide-in-from-bottom duration-200">
          <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <button @click="deleteInvoiceTarget = null" class="text-slate-400 hover:text-slate-700 font-bold text-lg cursor-pointer">✕</button>
          </div>
          ...
          <div class="space-y-3">
            ...
            <div class="p-3.5 bg-amber-50 border border-amber-200 rounded-2xl text-amber-900 space-y-1 text-xs">
              <span class="font-black block">⚠️ Warning:</span>
              <p class="font-medium text-[11px] leading-relaxed">
                This action will permanently reverse stock movements, restore item inventory levels, and update daily revenue totals.
              </p>
            </div>

            <div v-if="deleteError" class="p-3 bg-red-50 border border-red-200 rounded-2xl font-bold text-red-700 text-xs leading-relaxed">
              {{ deleteError }}
            </div>
          </div>
...
async function fetchInvoices(page = 1) {
  ...
  try {
    ...
    const itemsList = payload.items || payload.data || (Array.isArray(payload) ? payload : []);
    const pagData = payload.pagination || payload.meta || null;

    if (Array.isArray(itemsList)) {
      invoices.value = itemsList;
      if (pagData) Object.assign(pagination, pagData);
    } else {
      invoices.value = [];
    }
  } catch (err) {
...
watch(
  [selectedInvoice, receiptInvoice, deleteInvoiceTarget],
  (modalStates) => {
