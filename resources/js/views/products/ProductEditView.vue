<template>
  <div class="space-y-6 max-w-5xl mx-auto pb-16 antialiased font-sans">
    ...
    <div class="flex items-center justify-between border-b border-slate-200/80 pb-4">
      ...
      <span v-if="form.article_number" class="px-3 py-1 bg-red-50 text-red-700 border border-red-200 font-mono text-xs font-black rounded-xl">
        ART: {{ form.article_number }}
      </span>
    </div>

    <div v-if="loading" class="text-center py-16 bg-white rounded-2xl border border-slate-200 text-slate-500 font-bold text-xs">
      Loading footwear article details from database...
    </div>
    ...
    <form v-else @submit.prevent="submitUpdate" class="space-y-6">
      ...
      <div class="bg-white rounded-2xl border border-slate-200/90 p-6 shadow-xs space-y-4">
        ...
        <div class="space-y-1 text-xs">
          <label class="font-bold text-slate-700 block">Description</label>
          <textarea
            v-model="form.description"
            rows="3"
            placeholder="Article features, styling description and details..."
        ...
        <div class="flex items-center gap-6 pt-2 text-xs">
          ...
          <label class="flex items-center gap-2 font-bold text-slate-700 cursor-pointer">
            ...
            <span>Active Article Status</span>
          </label>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-slate-200/90 p-6 shadow-xs space-y-4">
        <div class="flex items-center gap-2 border-b border-slate-100 pb-3">
          <span class="text-base">📸</span>
          <h2 class="text-xs font-black text-slate-900 uppercase tracking-wider">Product Image Management</h2>
        </div>
          ...
          >
            ...
            <div v-if="!imagePreviewUrl" class="space-y-2">
              ...
              <p class="font-black text-slate-800 text-xs">
                Click to browse or drag & drop new product image here
              </p>
...
async function onCategoryChange() {
  ...
  try {
    ...
    if (res.data && res.data.id && !form.size_chart_id) {
...
async function updateAvailableSizes() {
  ...
  if (currentChecked.size === 0) {
    ...
    const validNumbers = new Set(availableSizes.value.map(s => s.size_number));
    selectedSizes.value = Array.from(currentChecked).filter(num => validNumbers.has(num));
...
async function fetchMasterDataAndProduct() {
  ...
  try {
    ...
    form.hsn_code_id = p.hsn_code_id || '';
    form.gender = p.gender || 'unisex';
    form.upper_material = p.upper_material || '';
    form.sole_material = p.sole_material || '';
    form.description = p.description || '';
    form.is_active = p.is_active !== false;
    form.is_visible_on_web = p.is_visible_on_web !== false;

    // Load primary image
    if (p.images && p.images.length > 0) {
      const primary = p.images.find(img => img.is_primary) || p.images[0];
      imagePreviewUrl.value = primary.image_path;
    }

    // Load existing variants & selected size numbers
    const existingSizeNumbers = new Set();
    if (p.variants && p.variants.length > 0) {
      form.color_blocks = p.variants.map(variant => {
        const firstSize = variant.sizes?.[0];
        if (firstSize) {
          form.mrp = firstSize.mrp || form.mrp;
          form.selling_price = firstSize.selling_price || form.selling_price;
          form.cost_price = firstSize.cost_price || form.cost_price;
        }
    ...
    if (existingSizeNumbers.size > 0) {
      selectedSizes.value = Array.from(existingSizeNumbers);
    }
...
async function submitUpdate() {
  ...
  try {
    ...
    const res = await api.post(`/products/${form.id}/bulk-update`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });

    toast.success(`Product ${form.article_number} (${form.name}) updated successfully!`);
    router.push('/admin/products');
  } catch (err) {
    console.error('Failed to update product:', err);
    toast.error(err.message || 'Failed to update product article.');
  } finally {
